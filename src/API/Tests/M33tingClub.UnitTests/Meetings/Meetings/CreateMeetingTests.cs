...
public class CreateMeetingTests : TestBase
{
	...
	{
		// Given
		var currentDate = new DateTimeOffset(2023, 6, 1, 9, 0, 0, DateTimeOffset.Now.Offset);
		...
		var tags = new List<TagInfo> { TagInfo.Create(TagName.Create("tag1")), TagInfo.Create(TagName.Create("tag2")) };
		var userId = UserId.FromGuid(Guid.Parse("ED23FD87-E493-4994-BCF7-A018972B84ED"));
		var isPublic = true;
		var confidentialInfo = "Some info";
		
		// When
		var func = () => Meeting.Create(
			meetingId,
			meetingName,
			meetingDescription,
			participantsLimit,
