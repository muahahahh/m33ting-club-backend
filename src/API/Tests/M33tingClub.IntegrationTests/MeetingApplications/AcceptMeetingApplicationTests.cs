...
[TestFixture]
internal class AcceptMeetingApplicationsTests : TestBase
{
	[Test]
	public async Task GivenPrivateMeetingWithOneApplication_WhenOwnerAcceptsApplication_ThenApplicationIsAccepted()
	{
		// Given
		...
		// When
		var response = await MeetingApplicationsClient.AcceptApplication(meetingId, applicant.Id, sergioUser);
			...
			new ApplicationDTO
				{
					MeetingId = meetingId,
					UserId = applicant.Id,
					Status = MeetingApplicationStatus.Accepted.Name,
					Name = applicant.Name,
		...
		var meetingNotifications = await UsersClient.GetUnseenMeetingNotifications(applicant.AuthToken);
		meetingNotifications.Content.Should().ContainEquivalentOf(new MeetingNotificationDTO
		{
			Type = MeetingNotificationType.ApplicationAccepted.Name,
			MeetingId = meetingId,
			PerformerId = sergioUser.Id,
			PerformerName = sergioUser.Name
		}, options => options
			.Excluding(x => x.Id)
	...
	[Test]
	public async Task GivenPrivateMeetingWithOneApplication_WhenMemberAcceptsApplication_ThenExceptionIsThrown()
	{
		// Given
		var sergioUser = TestAuthUsersProvider.SergioPerez;
